...
function Form2({
    ...
    setFormData((prevData) => ({
      ...
      useEffect(() => {      
        ...
        setFormData((prevData) => {
          const updatedData = {
            ...prevData,
            chargeReason: rowData.chargeReason || "",
            billing: rowData.billing || "",
            encrumbrance: rowData.encrumbrance || "",
            ...
            netAmount: rowData.finalAmount
              ? String(rowData.finalAmount).replace(/[₹,]/g, "").trim()
              : "",
        ...
        if (!Array.isArray(rowData.receiptAmount) || rowData.receiptAmount.length === 0) {
          return;
        }
        ...
        setFields((prevFields) => {      
          const updatedFields = {
            ...
            receiptAmount: rowData.receiptAmount.map((item, idx) => {
              const extractedTotalAmount = item.totalAmount
                ? String(item.totalAmount).replace(/[₹,]/g, "").trim()
                : "0.00";      
              return {
                totalAmount: extractedTotalAmount, 
      
                paymentDetails: Array.isArray(item.paymentDetails)
                  ? item.paymentDetails.map((detail, detailIdx) => {
                      const amount = detail.amount
                        ? parseFloat(String(detail.amount).replace(/[₹,]/g, "").trim()) || 0
                        : 0;
                      return {
  ...
  const handleInputValidation = (e) => {
    ...
    if (
    ...
    else if (name === "chequeRTGSorNEFT" || name === "payerAddress") {
      ...
      if (!/^\d*\.?\d{0,2}$/.test(value) || /^0/.test(value)) {
        ...
        error =
          validValue === "" ? "This field cannot start with 0" : "Only numbers and up to two decimal places are allowed";
        isValid = false;
      }
    }
    ...
    const dataToSend = {
      hierarchyData: {
        ...
        additionalFields: {
          ...
          encrumbrance: formData.encrumbrance,
      ...
      <form>
        <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col">
          ...
          <div className="-mx-3 md:flex mb-6">
            <div className="md:w-1/2 px-3 mb-6 md:mb-0">
              <label className="tracking-wide text-black text-xs font-bold mb-2">
                Fees Charged against property/Other
              </label>
              <input
                name="encrumbrance"
                value={formData.encrumbrance}
                onChange={handleInputValidation}
                  onBlur={handleInputValidation}
                onFocus={handleFocus}
                className="w-full bg-gray-200 text-black border border-gray-200 rounded py-3 px-4 mb-3"
                type="text"
                placeholder="Fees Charged against property/Other"
                maxLength={20}
              />
              {errors.encrumbrance && (
                <p className="text-red-500 text-xs italic">
